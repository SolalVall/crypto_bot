...
base_crypto = [
    [{"name": "Bitcoin", "trigramme": "BTC"}],
    [{"name": "Ethereum", "trigramme": "ETH"}],
    [{"name": "Litecoin", "trigramme": "LTC"}],
    [{"name": "Ripple", "trigramme": "XRP"}],
    [{"name": "EOS", "trigramme": "EOS"}],
]
        ...
        if request.form["validate"] == "Create Account":
            ...
            if error_register:
                return render_template(
                    "register.html", error=error, error_register=error_register
                )
            else:
    ...
    if request.method == "POST":
        if request.form["connection"] == "Connect":
            ...
            if type(check_login) == str:
                return render_template(
                    "login.html", error=error, check_login=check_login
                )
            else:
    ...
    if request.method == "POST":
        if request.form["bttnInfo"] == "Get Info":
            ...
            selected_crypto.url = (
                ...
                + selected_crypto.trigramme
                + "&tsyms="
                + selected_crypto.devise
            )

            cryptoInfo = requests.get(selected_crypto.url)
            cryptoInfoJson = cryptoInfo.json()

            selected_crypto.price = cryptoInfoJson["RAW"][selected_crypto.trigramme][
                selected_crypto.devise
            ]["PRICE"]
            selected_crypto.openDay = cryptoInfoJson["RAW"][selected_crypto.trigramme][
                selected_crypto.devise
            ]["OPENDAY"]
            selected_crypto.highDay = cryptoInfoJson["RAW"][selected_crypto.trigramme][
                selected_crypto.devise
            ]["HIGHDAY"]
            selected_crypto.lowDay = cryptoInfoJson["RAW"][selected_crypto.trigramme][
                selected_crypto.devise
            ]["LOWDAY"]
            selected_crypto.marketCap = cryptoInfoJson["RAW"][
                selected_crypto.trigramme
            ][selected_crypto.devise]["MKTCAP"]
            selected_crypto.evolutionDay = cryptoInfoJson["RAW"][
                selected_crypto.trigramme
            ][selected_crypto.devise]["CHANGEPCTDAY"]

            return render_template(
                "configBot.html", selected_crypto=selected_crypto, error=error
            )
    else:
...
def checkPrice():
    ...
    if request.method == "POST":
        try:
            check_duration = selected_crypto.check_duration(
            ...
            if type(check_duration) == str:
                ...
                return render_template(
                    "checkPrice.html", selected_crypto=selected_crypto, error=error
                )
        except ValueError:
            notInt = "Please you have to enter only numeric values [0-9]"
            return render_template(
                "bot.html", notInt=notInt, selected_crypto=selected_crypto, error=error
            )
    # Actualize bot GET request
    else:
        selected_crypto.price = selected_crypto.updatedPrice
        new_price = selected_crypto.update_price(
            selected_crypto.url, selected_crypto.trigramme, selected_crypto.devise
        )
        ...
        return jsonify(
            new_price=new_price,
            previousPrice=selected_crypto.price,
            comparePrice=selected_crypto.comparePrice,
            variation=variation,
        )
